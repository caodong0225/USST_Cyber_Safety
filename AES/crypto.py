...
def string_to_hex(text : str):
    ...
    # 计算需要的字节数，确保是16字节的倍数
    needed_length = 16 * ((len(byte_data) + 15) // 16)

    # 如果不足16字节或其倍数，进行PKCS#7填充
    if len(byte_data) < needed_length:
        padding_length = needed_length - len(byte_data)
        byte_data = byte_data + bytes([padding_length]) * padding_length

    # 转换为十六进制整数
    hex_int = int.from_bytes(byte_data, byteorder='big')

    return hex_int


def hex_to_string(hex_int : int):
    """将十六进制整数转换回文本字符串
    ...
    """
    # 转换为字节
    byte_data = hex_int.to_bytes((hex_int.bit_length() + 7) // 8, byteorder='big')

    # 检查并移除PKCS#7填充
    padding_value = byte_data[-1]
    if padding_value < 16 and all(b == padding_value for b in byte_data[-padding_value:]):
        byte_data = byte_data[:-padding_value]
